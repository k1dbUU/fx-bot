...
async def exec_trade(conn, sym: str, direction: str,
    ...
    try:
        opts = {"comment": f"{comment} {sym}", "clientId": f"KC{MAGIC}"}
        if direction == "BUY":
...
async def run_bot():
    log.info("══════════════════════════════════════════")
    log.info("KC FX Bot v5.1 — ONLINE — GitHub Actions")
    log.info("══════════════════════════════════════════")
    ...
    try:
        ...
        log.error(f"[META] Sync timeout after {SYNC_TIMEOUT_SECONDS}s — aborting this cycle")
        await conn.close()
        save_state(state)
        return
    ...
    for sym in SYMBOLS:
        ...
        if total_open >= MAX_OPEN_TRADES:
            log.info(f"[GLOBAL] Max open trades reached — stopping")
            break
        ...
        # Candles — "4h" is the correct MetaAPI timeframe (was "1h4", now fixed)
        try:
        ...
        # Use previous completed day for daily levels (not partial current day)
        if len(c_d1) >= 2:
            dh = c_d1[-2]["high"]
            dl = c_d1[-2]["low"]
        elif c_d1:
        ...
        if sb_win:
            ...
            if spd > MAX_SPREAD_SB:
                log.info(f"[{sym}][SB] Spread {spd}pts exceeds cap {MAX_SPREAD_SB}pts — skip")
            elif bias == "NEUTRAL":
        ...
        if in_sweep and spd <= MAX_SPREAD_SMC:
            ...
            log.info(f"[{sym}][SMC] Spread {spd}pts exceeds SMC cap {MAX_SPREAD_SMC}pts")
        else:
